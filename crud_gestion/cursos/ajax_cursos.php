...
if ($action === 'guardar_cambios') {
    ...
    foreach ($data as $row) {
        ...
        $id = intval($row['id'] ?? $row['id_curso'] ?? 0);
        if ($id <= 0) continue;
...
if ($action === 'borrar_varios') {
    ...
    $stmt = $conexion->prepare("DELETE FROM cursos WHERE id_curso IN ($placeholders)");
...
if ($action === 'borrar_todos') {
    $conexion->query("TRUNCATE TABLE cursos");
    echo json_encode(['status'=>'ok','msg'=>'Todos los cursos eliminados']);
    exit;
}
